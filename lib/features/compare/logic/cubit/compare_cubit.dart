...
class CompareCubit extends Cubit<CompareState> {
  ...
  Future<void> compareSelectedOffers() async {
    // Allow extracting best offers from a single file as well as
    // multi-file comparisons. Backend's matching service handles
    // both cases — we just need to send at least one offer id.
    if (selectedCount < 1) {
      return;
    }
  ...
  Future<void> generateProposal() async {
    // Same single-or-multi support as compareSelectedOffers.
    if (selectedCount < 1) {
      return;
    }
