...
class SidebarWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // When rendered inside a [Drawer] on mobile, expand to fill the drawer
    // width. On desktop, keep the original 220.w fixed width.
    final isInDrawer = Scaffold.maybeOf(context)?.hasDrawer == true &&
        MediaQuery.of(context).size.width < 768;
    return Container(
      width: isInDrawer ? double.infinity : 220.w,
  ...
  Widget _buildMenuItem({
    ...
    return InkWell(
      onTap: () {
        ...
        // Close drawer first if we're rendered inside one (mobile),
        // so the navigation animation feels smooth.
